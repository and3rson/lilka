...
//  2022: modified by @yasserbdj96
//  2023: modified by @and3rson (cppcheck & clang-format)
...
void FtpServer::clientConnected() {
    ...
    client.println("220---   By yasserbdj96   ---");
...
boolean FtpServer::userIdentity() {
    ...
    if (strcmp(parameters, _FTP_USER.c_str())) client.println("530 user not found");
    else {
...
boolean FtpServer::userPassword() {
    if (strcmp(command, "PASS")) client.println("500 Syntax error");
    else if (strcmp(parameters, _FTP_PASS.c_str())) client.println("530 ");
    else {
#ifdef FTP_DEBUG
...
//data = dataServer.available();
    ...
    //
        ...
        p = strchr(p, ',');
        dataPort += atoi(++p);
        if (p == NULL) client.println("501 Can't interpret parameters");
        else {
    ...
    //
        ...
        if (strlen(parameters) == 0) client.println("501 No file name");
        else if (makePath(path)) {
            if (!SD.exists(path)) client.println("550 File " + String(parameters) + " not found");
            else {
                if (SD.remove(path)) client.println("250 Deleted " + String(parameters));
    ...
    //
        ...
        if (!dataConnect()) client.println("425 No data connection");
        else {
            ...
            if ((!dir) || (!dir.isDirectory())) client.println("550 Can't open directory " + String(cwdName));
            else {
                File iterFile = dir.openNextFile();
                while (iterFile) {
                    String fn, fs;
                    fn = iterFile.name();
                    //fn.remove(0, 1);
...
#endif
                    fs = String(iterFile.size());
                    if (iterFile.isDirectory()) {
                        data.println("<DIR> " + fn);
                    } else {
                        data.println("<FILE> " + fs + " " + fn);
                        //          data.println( " " + fn );
                    }
                    nm++;
                    iterFile = dir.openNextFile();
                }
    ...
    //
        ...
        if (!dataConnect()) client.println("425 No data connection MLSD");
        else {
            client.println("150 Accepted data connection");
            //      Dir dir= SD.openDir(cwdName);
            File dir = SD.open(cwdName);
    ...
    //
        ...
        if (!dataConnect()) client.println("425 No data connection");
            ...
            //      Dir dir=SD.openDir(cwdName);
            File dir = SD.open(cwdName);
            if (!SD.exists(cwdName)) client.println("550 Can't open directory " + String(parameters));
            else {
                File iterFile = dir.openNextFile();
                uint16_t nm = 0;
                //        while( dir.next())
    ...
    //
        ...
        if (strlen(parameters) == 0) client.println("501 No file name");
            ...
            file = SD.open(path, "r");
            if (!file) client.println("550 File " + String(parameters) + " not found");
            else if (!dataConnect()) client.println("425 No data connection");
            else {
#ifdef FTP_DEBUG
    ...
    //
        ...
        if (strlen(parameters) == 0) client.println("501 No file name");
            ...
            file = SD.open(path, "w");
            if (!file) client.println("451 Can't open/create " + String(parameters));
            else if (!dataConnect()) {
    ...
    //
        ...
        if (strlen(parameters) == 0) client.println("501 No file name");
            ...
            if (!SD.exists(buf)) client.println("550 File " + String(parameters) + " not found");
            else {
#ifdef FTP_DEBUG
    ...
    //
        ...
        if (strlen(buf) == 0 || !rnfrCmd) client.println("503 Need RNFR before RNTO");
            ...
            if (SD.exists(path)) client.println("553 " + String(parameters) + " already exists");
            else {
#ifdef FTP_DEBUG
                Serial.println("Renaming " + String(buf) + " to " + String(path));
#endif
                if (SD.rename(buf, path)) client.println("250 File successfully renamed or moved");
                else client.println("451 Rename/move failure");
            }
        }
        rnfrCmd = false;
    }
    ...
    //
        ...
        if (strlen(parameters) == 0) client.println("501 No file name");
            ...
            file = SD.open(path, "r");
            if (!file) client.println("450 Can't open " + String(parameters));
            else {
...
boolean FtpServer::doRetrieve() {
    ...
    if (nb > 0) {
        data.write(reinterpret_cast<uint8_t*>(buf), nb);
        bytesTransfered += nb;
        return true;
    }
    closeTransfer();
...
boolean FtpServer::doStore() {
    if (data.connected()) {
        int16_t nb = data.readBytes(reinterpret_cast<uint8_t*>(buf), FTP_BUF_SIZE);
        if (nb > 0) {
            // Serial.println( millis() << " " << nb << endl;
            file.write(reinterpret_cast<uint8_t*>(buf), nb);
            bytesTransfered += nb;
        }
        return true;
    }
        ...
        if (c != '\r')
            if (c != '\n') {
                ...
                if (iCL == 0) rc = 0;
                    ...
                    if (parameters != NULL) {
                        if (parameters - cmdLine > 4) rc = -2; // Syntax error
                        else {
                    ...
                    } else if (strlen(cmdLine) > 4) rc = -2; // Syntax error.
