...
static int lualilka_imageTransform_multiply(lua_State* L){
	lilka::Transform* transformPtr = reinterpret_cast<lilka::Transform*>(luaL_checkudata(L, 1, IMAGE_TRANSFORM));
	lilka::Transform multipliedTransfform = transformPtr->multiply(*reinterpret_cast<lilka::Transform*>(luaL_checkudata(L, 2, IMAGE_TRANSFORM)));
...
static int lualilka_imageTransform_vtransform(lua_State* L){
    lilka::int_vector_t result = (*reinterpret_cast<lilka::Transform**>(luaL_checkudata(L, 1, IMAGE_TRANSFORM)))->transform(lilka::int_vector_t {.x = (int) luaL_checknumber(L, 2), .y = (int) luaL_checknumber(L, 3)});
...
static int lualilka_imageTransform_get_matrix(lua_State* L){
	...
	for (int i = 0; i < 2; ++i) {
		lua_newtable(L);
		for (int j = 0; j < 2; ++j) {
			lua_pushinteger(L, (*reinterpret_cast<lilka::Transform**>(luaL_checkudata(L, 1, IMAGE_TRANSFORM)))->matrix[i][j]);
			lua_rawseti(L, -2, j + 1);
		}
		lua_rawseti(L, -2, i + 1);
	}
...
if (!lua_istable(L, 1)) {
	...
	for (int i = 0; i < 2; ++i) {
		...
		for (int j = 0; j < 2; ++j) {
			...
			(*reinterpret_cast<lilka::Transform**>(luaL_checkudata(L, 1, IMAGE_TRANSFORM)))->matrix[i][j] = lua_tonumber(L, -1);
			lua_pop(L, 1);
		}
...
int lualilka_imageTransform_register(lua_State* L) {
	...
	lua_pushcfunction(L, lualilka_imageTransform_set_matrix); lua_setfield(L, -2, "set");
	lua_pushcfunction(L, lualilka_imageTransform_get_matrix); lua_setfield(L, -2, "get");

	lua_pushcfunction(L, lualilka_imageTransform_scale); lua_setfield(L, -2, "scale");
	lua_pushcfunction(L, lualilka_imageTransform_rotate); lua_setfield(L, -2, "rotate");
	lua_pushcfunction(L, lualilka_imageTransform_multiply); lua_setfield(L, -2, "multiply");
	lua_pushcfunction(L, lualilka_imageTransform_inverse); lua_setfield(L, -2, "inverse");
	lua_pushcfunction(L, lualilka_imageTransform_vtransform); lua_setfield(L, -2, "vtransform");
