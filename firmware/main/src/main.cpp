...
public:
    ...
    void run() {
        while (1) {
            ...
            while (1) {
                cursor = lilka::ui_menu(canvas, "Головне меню", menu, count, cursor, icons, colors);
                if (cursor == 0) {
                    demos_menu();
                } else if (cursor == 1) {
                    sd_browser_menu("/");
                    ...
                    lilka::ui_alert(canvas, "Лілка Main OS", "by Андерсон\n& friends");
                }
            }
        }
    }
    ...
    void demos_menu() {
        ...
        int cursor = 0;
        while (1) {
            cursor = lilka::ui_menu(canvas, "Оберіть демо:", demos, count, cursor);
            if (!demo_funcs[cursor]) {
                return;
            }
            demo_funcs[cursor](canvas);
        }
    }
