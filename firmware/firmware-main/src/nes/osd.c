...
void osd_getinput(void)
{
	...
	for (x = 0; x < 16; x++)
	{
		if (chg & 1)
		{
			evh = event_get(ev[x]);
			if (evh) {
                nofrendo_log_printf("evh: %s, %d, %d\n", (b & 1) ? "break" : "make", x, ev[x]);
...
int osd_init()
{
	...
	// xTaskCreatePinnedToCore(displayTask, "displayTask", 2048, NULL, 10, NULL, 0);
    // xTaskCreate(&displayTask, "displayTask", 2048, NULL, 0, NULL);
	osd_initinput();
	return 0;
}
